...
| D7 | Découpage monorepo | `apps/web` (Next 16), `packages/data` (YAML + schéma + chargeur + contrôle d'intégrité), `packages/domain` (modèle d'actionnariat en TS pur), `packages/config` (tsconfig/lint partagés) | Le contrôle d'intégrité des données tourne en CI sans construire le site ; le domaine est testable hors Next. `packages/data` prévu pour accueillir un import Monde diplo (issue #11) plus tard ; aucune autre app décidée pour l'instant. |
...
_(remplies au fur et à mesure)_
